<script lang="ts">
    ...
    let wallet = $state({ name: 'Loading...', wallet_Address: '0x...', role: 'Agent' });
    let items = $state([]);
    ...
    async function handleValidateRecord() {
        ...
        try {
            ...
            const response = await fetch(`/api/agents/${agentId}/transaction`, {
                ...
                body: JSON.stringify({
                    items: [{ item_Id: selectedItemId, qty: quantity }]
                })
            });
            ...
            if (result.success) {
                const { payload, server_signature } = result.data;
                ...
                const tx = await contract.record_sale(payload, server_signature);
                await tx.wait();
                */
